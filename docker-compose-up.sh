...
CHECK_SCRIPT="$SCRIPT_DIR/check-postgres-port.sh"
...
# Check if port checking script exists
if [ -f "$CHECK_SCRIPT" ] && [ -x "$CHECK_SCRIPT" ]; then
    PORT_INFO=$($CHECK_SCRIPT)
    export POSTGRES_HOST_PORT=$(echo "$PORT_INFO" | grep "POSTGRES_HOST_PORT" | cut -d'=' -f2)
    
    if [ -n "$POSTGRES_HOST_PORT" ]; then
        echo "[*] Using PostgreSQL host port: $POSTGRES_HOST_PORT"
    else
        POSTGRES_HOST_PORT=$DEFAULT_PORT
        echo "[*] Using default PostgreSQL host port: $POSTGRES_HOST_PORT"
    fi
else
    # Fallback: try to check port manually
