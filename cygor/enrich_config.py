...
SOURCES = {
    ...
    "spur": {
        "name": "Spur",
        "env_var": "SPUR_API_KEY",
        "url": "https://spur.us/app/context",
        "test_url": "https://api.spur.us/v2/context/8.8.8.8"
    },
    "bazaar": {
        "name": "MalwareBazaar (abuse.ch)",
        "env_var": "BAZAAR_API_KEY",
        "url": "https://bazaar.abuse.ch/api/",
        "test_url": "https://mb-api.abuse.ch/api/v1/"
    },
    "prospeo": {
        "name": "Prospeo.io",
        "env_var": "PROSPEO_API_KEY",
        "url": "https://prospeo.io/api",
        "test_url": "https://api.prospeo.io/domain-search"
    },
    "crt_sh": {
        "name": "crt.sh (Certificate Transparency)",
        "env_var": None,            # public endpoint, no key
        "url": "https://crt.sh/",
        "test_url": "https://crt.sh/?q=example.com&output=json",
...
def test_api_key(source: str, api_key: str) -> tuple[bool, Optional[str]]:
    ...
    try:
        if source == "shodan":
            ...
            # MalwareBazaar API - test with query_hash endpoint
            url = "https://mb-api.abuse.ch/api/v1/"
            # Note: Bazaar API key is optional for most endpoints
            # Test by querying recent samples
            ...
            if response.status_code == 200:
                try:
                    ...
                    if result.get("query_status") == "ok":
                        ...
                        return False, result.get("query_status", "Unknown error")
                except:
            ...
            # Prospeo API - test with simple domain search
            ...
            if response.status_code == 200:
                ...
                return False, "Payment required - Check your subscription (402)"
            elif response.status_code == 429:
                return False, "Rate limit exceeded (429 Too Many Requests)"
            try:
                error_data = response.json()
...
Available sources: shodan, virustotal, abuseipdb, otx, urlscan, censys, greynoise, spur, dehashed, bazaar, prospeo

Note:
  - Censys requires API ID:SECRET format (e.g., xxxxx-xxxx-xxxx-xxxx:yyyyyyyyyyyyyyyy)
  - Dehashed requires email:api_key format (e.g., user@example.com:your_api_key)
  - Bazaar (MalwareBazaar) API key is optional for most queries (use "none" if no key)
  - Prospeo requires a valid API key for email/domain searches
        """
    )
