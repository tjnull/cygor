...
def get_module_output_dir(module_name: str = "template_module") -> Path:
    ...
    The workspace comes from $CYGOR_WORKSPACE/$CYGOR_RESULTS_DIR or the active
    workspace config; there is no implicit ./results default.
    """
    from cygor.workspace import require_workspace
    base = require_workspace() / "cygor-enumeration-modules" / module_name
...
def resolve_output_dir(cli_output_dir: str | None, module_name: str) -> Path:
    ...
    if cli_output_dir and cli_output_dir not in ("", None):
        ...
        from cygor.workspace import require_workspace
        outdir = require_workspace() / "cygor-enumeration-modules" / module_name / ts

    else:
        from cygor.workspace import require_workspace
        outdir = require_workspace() / "cygor-enumeration-modules" / module_name
...
def get_context(request=None, session=None):
    ...
    summary = {
        ...
        "Workspace Mode": os.environ.get("CYGOR_WORKSPACE") or os.environ.get("CYGOR_RESULTS_DIR") or "disabled",
