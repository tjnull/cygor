...
def _resolve_nmap_inputs(args) -> List[str]:
    ...
    # 3) auto-detect <workspace>/nmap
    from cygor.workspace import resolve_workspace
    ws = resolve_workspace()
    if not ws:
        print("[-] No --inputs/--nmap-dir given and no workspace set to auto-detect nmap output.")
        print("    Specify --inputs/--nmap-dir, pass -o, or set an active workspace (cygor workspace).")
        return []
    base = ws / "nmap"
    if not base.exists():
        print(f"[-] No directory supplied and default {base}/ not found.")
        return []
...
def add_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument("-o", "--out-dir", default=None, help="Output root (workspace). Required unless an active workspace is set with 'cygor workspace' or $CYGOR_WORKSPACE.")
...
def exec_argv(argv: List[str]):
    ...
    parser = argparse.ArgumentParser(
        prog="cygor parse",
        description="Cygor Parser (auto-detect workspace nmap output; enriched outputs; non-breaking)"
    )
    add_arguments(parser)
    args = parser.parse_args()

    # Resolve the output workspace (no implicit ./results default).
    from cygor.workspace import require_workspace
    outdir = require_workspace(args.out_dir)
    _ensure_dir(outdir)
